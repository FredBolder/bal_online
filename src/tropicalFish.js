...
export function drawFish(ctx, xc, yc, size, flipHorizontally, fish) {
    const { palette, shape, tail, fins, stripes, eyePercentage, pupilPercentage, eyeOffsetX, eyeOffsetY } = fish;
    if (flipHorizontally) {
        ctx.save();
        ctx.translate(xc, 0);
        ctx.scale(-1, 1);
    ...
    // Color palettes
    const colors = getTropicalFishColors(palette);
    ...
    switch (shape) {
        ...
        case 11:
            // Rusty Jobfish
            bodyCurvature = {
                topFrontBodyCpPos: 0.6,
                topFrontBodyCpDist: 0.25,
                topRearBodyCpPos: 0.7,
                topRearBodyCpDist: 0.05,
                bottomFrontBodyCpPos: 0.6,
    ...
    // ---- Pectoral fins ----
    if (!globalVars.debug) {
        drawForegroundFins(ctx, fins, yc, bodyHeight, bodyLength, bodyRight, bodyCurves, colors);
    }
    ...
    ctx.lineWidth = 1;
    let xEye = bodyOptions.isTang ? geom.headRight - bodyLength * 0.04 : geom.headRight - bodyLength * 0.15;
    xEye = xEye + (bodyLength * eyeOffsetX * 0.002);
    ...
    ctx.beginPath();
    ctx.arc(xEye, yEye, eyeRadius * pupilPercentage * 0.01, 0, Math.PI * 2);
    ctx.fill();
...
export function isClownFish(fish) {
    return ([8, 34].includes(fish.palette) && (fish.shape === 2) && (fish.tail === 6) && (fish.fins === 1) && (fish.stripes === 15));
}
...
export function moveTropicalFish(backData, gameData, gameInfo, gameVars) {
    ...
    for (let i = 0; i < gameInfo.tropicalFish.length; i++) {
        ...
        const clownFish = isClownFish(fish);
        goToFood = false;
        ...
        if (goToFood) {
            ...
            fish.blocked = false;
            if (fish.y > foodY) {
                if (freeToSwim(foodX, fish.x, fish.y - 1, backData, gameData, clownFish) && freeToSwim(foodX, fish.x, fish.y, backData, gameData, clownFish)) {
                    up = true;
                }
            }
            if (fish.y < foodY) {
        ...
        if (up && !nextToFood(gameData, fish.x, fish.y)) {
            if (fish.y > 0) {
                if ((gameData[fish.y - 1][fish.x] === 0) && canGo(backData, fish.x, fish.y - 1, clownFish)) {
                    fish.y--;
                } else if (!changed) {
                    fish.blocked = true;
                }
