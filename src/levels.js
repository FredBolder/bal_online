...
export const seriesChroniaPollaEnd = 994;
          ...
          switch (name) {
            ...
            case "$eyeoffsety":
              ...
              if ((val_int < -50) || (val_int > 50)) {
                msg += `${settingNr(i)}Invalid value ${values[0]} for eye offset.\n`;
              }
              break;
            case "$eyepercentage":
              ...
              switch (data[y][x]) {
                ...
                case 243:
                  ...
                  if ((val_int < 0) || (val_int > tropicalFishStripes)) {
                    msg += `${settingNr(i)}Invalid value ${values[2]} for stripes.\n`;
                  }
                  break;
                default:
                  msg += `${settingNr(i)}No tropical fish found at the coordinates ${x}, ${y}.\n`;
...
export function displayLevelNumber(level, addInfo = false) {
  ...
  if ((level >= series1Start) && (level <= series1End)) {
    ...
    series = "Fish";
  } else if ((level >= seriesProgrammingStart) && (level <= seriesProgrammingEnd)) {
  ...
  for (let i = 0; i < levelSettings.length; i++) {
    ...
    if (p1 >= 0) {
      ...
      switch (name) {
        ...
        case "$eyeoffsetx":
          val_int = tryParseInt(values[2], -1);
          setProp(gameData, gameInfo, x, y, "eyeOffsetX", val_int, false);
          break;
        case "$eyeoffsety":
          val_int = tryParseInt(values[2], -1);
